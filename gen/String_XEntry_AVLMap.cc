///////////////////////////////////////////////////////////////////////////////
// $Id: String_XEntry_AVLMap.cc,v 1.1.1.1 1993/02/04 15:18:43 ak Exp $
///////////////////////////////////////////////////////////////////////////////
// $Log: String_XEntry_AVLMap.cc,v $
// Revision 1.1.1.1  1993/02/04  15:18:43  ak
// Please Edit this file to contain the RCS log information
// to be associated with this file (please remove these lines)
//
// Revision 1.1  1993/02/04  15:18:40  ak
// Initial revision
//
///////////////////////////////////////////////////////////////////////////////

static char *rcsid = "$Id: String_XEntry_AVLMap.cc,v 1.1.1.1 1993/02/04 15:18:43 ak Exp $";
...
static inline int bf(StringXEntryAVLNode* t)
{
  return t->stat & AVLBALANCEMASK;
}

static inline void set_bf(StringXEntryAVLNode* t, int b)
{
  t->stat = (t->stat & ~AVLBALANCEMASK) | (b & AVLBALANCEMASK);
}


static inline int rthread(StringXEntryAVLNode* t)
{
  return t->stat & RTHREADBIT;
}

static inline void set_rthread(StringXEntryAVLNode* t, int b)
{
  if (b)
    t->stat |= RTHREADBIT;
  else
...
static inline int lthread(StringXEntryAVLNode* t)
{
  return t->stat & LTHREADBIT;
}

static inline void set_lthread(StringXEntryAVLNode* t, int b)
{
  if (b)
    t->stat |= LTHREADBIT;
  else
...
StringXEntryAVLNode* StringXEntryAVLMap::leftmost()
{
  StringXEntryAVLNode* t = root;
  if (t != 0) while (t->lt != 0) t = t->lt;
  return t;
}

StringXEntryAVLNode* StringXEntryAVLMap::rightmost()
{
  StringXEntryAVLNode* t = root;
...
StringXEntryAVLNode* StringXEntryAVLMap::succ(StringXEntryAVLNode* t)
{
  StringXEntryAVLNode* r = t->rt;
  if (!rthread(t)) while (!lthread(r)) r = r->lt;
  return r;
}

StringXEntryAVLNode* StringXEntryAVLMap::pred(StringXEntryAVLNode* t)
{
  StringXEntryAVLNode* l = t->lt;
  if (!lthread(t)) while (!rthread(l)) l = l->rt;
  return l;
}


Pix StringXEntryAVLMap::seek(String& key)
{
  StringXEntryAVLNode* t = root;
  if (t == 0)
    return 0;
...
static StringXEntryAVLNode* _found_node; // returned added/deleted node
static int    _already_found;   // for deletion subcases


void StringXEntryAVLMap:: _add(StringXEntryAVLNode*& t)
{
  ...
  else if (cmp < 0)
  {
    ...
    {
      ++count;
      _found_node = new StringXEntryAVLNode(*_target_item, def);
...
XEntry& StringXEntryAVLMap::operator [] (String& item)
{
  if (root == 0)
  {
    ++count;
    root = new StringXEntryAVLNode(item, def);
    set_rthread(root, 1);
    set_lthread(root, 1);
    return root->cont;
  }
...
void StringXEntryAVLMap::_del(StringXEntryAVLNode* par, StringXEntryAVLNode*& t)
{
  int comp;
  if (_already_found)
  {
...
void StringXEntryAVLMap::del(String& item)
{
  if (root == 0) return;
  _need_rebalancing = 0;
  _already_found = 0;
...
void StringXEntryAVLMap::_kill(StringXEntryAVLNode* t)
{
  if (t != 0)
  {
...
StringXEntryAVLMap::StringXEntryAVLMap(StringXEntryAVLMap& b) :StringXEntryMap(b.def)
{
  root = 0;
  count = 0;
...
int StringXEntryAVLMap::OK()
{
  ...
  {
    int n = 1;
    StringXEntryAVLNode* trail = leftmost();
    StringXEntryAVLNode* t = succ(trail);
    while (t != 0)
    {
      ++n;
