///////////////////////////////////////////////////////////////////////////////
// $Id: String_Name_AVLMap.cc,v 1.1.1.1 1993/02/04 15:18:02 ak Exp $
///////////////////////////////////////////////////////////////////////////////
// $Log: String_Name_AVLMap.cc,v $
// Revision 1.1.1.1  1993/02/04  15:18:02  ak
// Please Edit this file to contain the RCS log information
// to be associated with this file (please remove these lines)
//
// Revision 1.1  1993/02/04  15:17:59  ak
// Initial revision
//
///////////////////////////////////////////////////////////////////////////////

static char *rcsid = "$Id: String_Name_AVLMap.cc,v 1.1.1.1 1993/02/04 15:18:02 ak Exp $";
...
static inline int bf(StringNameAVLNode* t)
{
  return t->stat & AVLBALANCEMASK;
}

static inline void set_bf(StringNameAVLNode* t, int b)
{
  t->stat = (t->stat & ~AVLBALANCEMASK) | (b & AVLBALANCEMASK);
}


static inline int rthread(StringNameAVLNode* t)
{
  return t->stat & RTHREADBIT;
}

static inline void set_rthread(StringNameAVLNode* t, int b)
{
  if (b)
    t->stat |= RTHREADBIT;
  else
...
static inline int lthread(StringNameAVLNode* t)
{
  return t->stat & LTHREADBIT;
}

static inline void set_lthread(StringNameAVLNode* t, int b)
{
  if (b)
    t->stat |= LTHREADBIT;
  else
...
StringNameAVLNode* StringNameAVLMap::leftmost()
{
  StringNameAVLNode* t = root;
  if (t != 0) while (t->lt != 0) t = t->lt;
  return t;
}

StringNameAVLNode* StringNameAVLMap::rightmost()
{
  StringNameAVLNode* t = root;
...
StringNameAVLNode* StringNameAVLMap::succ(StringNameAVLNode* t)
{
  StringNameAVLNode* r = t->rt;
  if (!rthread(t)) while (!lthread(r)) r = r->lt;
  return r;
}

StringNameAVLNode* StringNameAVLMap::pred(StringNameAVLNode* t)
{
  StringNameAVLNode* l = t->lt;
  if (!lthread(t)) while (!rthread(l)) l = l->rt;
  return l;
}


Pix StringNameAVLMap::seek(String& key)
{
  StringNameAVLNode* t = root;
  if (t == 0)
    return 0;
...
static StringNameAVLNode* _found_node; // returned added/deleted node
static int    _already_found;   // for deletion subcases


void StringNameAVLMap:: _add(StringNameAVLNode*& t)
{
  ...
  else if (cmp < 0)
  {
    ...
    {
      ++count;
      _found_node = new StringNameAVLNode(*_target_item, def);
...
Name& StringNameAVLMap::operator [] (String& item)
{
  if (root == 0)
  {
    ++count;
    root = new StringNameAVLNode(item, def);
    set_rthread(root, 1);
    set_lthread(root, 1);
    return root->cont;
  }
...
void StringNameAVLMap::_del(StringNameAVLNode* par, StringNameAVLNode*& t)
{
  int comp;
  if (_already_found)
  {
...
void StringNameAVLMap::del(String& item)
{
  if (root == 0) return;
  _need_rebalancing = 0;
  _already_found = 0;
...
void StringNameAVLMap::_kill(StringNameAVLNode* t)
{
  if (t != 0)
  {
...
StringNameAVLMap::StringNameAVLMap(StringNameAVLMap& b) :StringNameMap(b.def)
{
  root = 0;
  count = 0;
...
int StringNameAVLMap::OK()
{
  ...
  {
    int n = 1;
    StringNameAVLNode* trail = leftmost();
    StringNameAVLNode* t = succ(trail);
    while (t != 0)
    {
      ++n;
